...
class Gate:
    ...
    def gate_gene(self, n, k, c=-1):
        a = np.identity(2 ** n, dtype=complex)
        if c == k:
            raise ValueError
        if c in range(n):
            con_range = [m * 2 ** (n - c) + 2 ** (n - c - 1) + i for m in range(2 ** c) for i in
                         range(2 ** (n - c - 1))]
        elif c == -1:
            con_range = range(2 ** n)
        else:
        ...
        for i_0 in con_range:
            if (i_0 // (2 ** (n - k - 1))) % 2 == 0:
                i_1 = i_0 + 2 ** (n - k - 1)
                a[i_0, i_0] = self.cell[0, 0]
                a[i_0, i_1] = self.cell[0, 1]
                a[i_1, i_0] = self.cell[1, 0]
                a[i_1, i_1] = self.cell[1, 1]
        return a
