...
for iter_times in range(0, iter_max):
    ...
    pos_[iter_times] = \
        round(q_vec.calc_pos_state((1, 1, 1, 1, 1, 1, 1)) +
              ...
              q_vec.calc_pos_state((1, 1, 1, 1, 0, 0, 1)), 4)

print(pos_)
theta = 2 * np.arccos(np.sqrt((N - M) / N))
x = np.linspace(0, iter_max, 1000)
y = (np.sin((x + 1 / 2) * theta)) ** 2
plt.plot(x, y, linestyle='--', label=u'理论')
plt.plot([i for i in range(0, iter_max)], pos_, 'ro', label=u'模拟数据')
plt.annotate('R = {}, p = {}'.format(R, pos_[R]),
             (R, pos_[R]), arrowprops=dict(arrowstyle='->'))
